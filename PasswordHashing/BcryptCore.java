...
import cologne.eck.dr.op.crypto.password_hashing.test.TestBcrypt;
 ...
 * 
 *  This implementation uses some parts of Bouncy Castles BlowfishEngine. 
 *  A slightly modified version of this implementation is now part
 *  of Bouncy Castles Crypto API. 
 *  
 */

public final class BcryptCore implements PasswordHashingScheme
{	
	...
	@Override
	// Note: To fit the interface, outlen is always 24 (192 bit), m_cost is always 0
	public byte[] hashPassword(int outlen, byte[] pwd, byte[] salt, int t_cost,
		...
		byte[] encryptedString = encryptMagicString();   
		
		// prevent dead code eliminations (compiler optimizations):
		if ((salt32Bit[
		          salt32Bit.length -1] 
				| salt32Bit2[salt32Bit2.length-1]
				| P[P.length -1]
				| S[S.length -1]) != 0) {
			System.err.print("zeroization failed!");
		}
