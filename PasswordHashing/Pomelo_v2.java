...
/*
 * Password Hashing Scheme Pomelo (Version 2)
...
/*
	...
	According to the author, POMELO can be used directly as KDF.  
	The output of POMELO is designed to be random and can be used directly as the key.
	Note: Version v3 introduces an iterative key derivation mode for this purpose.  
	
	Client-independent update:
	Use the algorithm with the new parameters to hash the old password image generated 
	with the old parameters. After the update, for any input password, the
	algorithm is applied twice: one with the old parameters; another with the new
	parameters.
...
import cologne.eck.dr.op.crypto.password_hashing.test.TestPomelo_v2;
